...
- 🖥️ **托管 / 接管 `dsh web`**：优先接管本机已运行的实例，否则自动拉起新实例；零配置开箱即用
...
- 应用启动后按顺序尝试：**接管** `127.0.0.1:<port>` 上已在运行的 dsh web（如浏览器里正开着的 `dsh web`）→ 无响应则**自起**新实例。
- 接管的实例退出/失联时自动重连，长时间失联后在同一端口原位自起。
